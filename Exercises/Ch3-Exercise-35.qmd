...
We can use the `$` operator to create new columns in the `survModelSummary` dataframe and extract any column from the dataframe by name. the `$` operator by default extracts a vector, which means vector operations can be done on them such as `[i]` to extract the *i*th element of the vector. If you want to refresh your memory on how to use `[i]` to extract a data element, review exercise 2.5. 
```{r 3.5.0 survival model 3}
#replace `columnname`
...
## Step 1: Calculate the transition probabilities
